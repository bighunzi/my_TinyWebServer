...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    m_url = strpbrk(text, " \t");//该函数返回 str1 中第一个匹配字符串 str2 中字符的 char *，如果未找到字符则返回 NULL。
    //如果没有空格或\t，则报文格式有误
    if (!m_url)
    {
        return BAD_REQUEST;
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    strcpy(m_real_file, doc_root);//通过调用http_conn::init定义doc_root
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        strcpy(m_url_real, "/");
        strcat(m_url_real, m_url + 2);//把 m_url + 2 所指向的字符串追加到 m_url_real 所指向的字符串的结尾。
...
/*
下面几个函数均是内部调用add_response函数更新m_write_idx指针和缓冲区m_write_buf中的内容。
*/
